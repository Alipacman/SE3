...
#|
Blatt 10, Gruppe 8, Mittwoch 10-12 Uhr, Raum G-210, Übungsleiter Rainer Jürgensen
...
; Anmerkung: Unter Windows funktionert die Funktion check-duplicates, unter Mac OS muss das s gelöscht werden (check-duplicate).
; Bitte je nach verwendetem System in den folgenden Hilfsfunktionen anpassen

(define (check-zeilen spiel)
  (map check-duplicate (map (curry remove* (list 0 'X)) (map (curry spiel->einträge spiel) (map zeile->indizes '(0 1 2 3 4 5 6 7 8))))))

(define (check-spalten spiel)
  (map check-duplicate (map (curry remove* (list 0 'X)) (map (curry spiel->einträge spiel) (map spalte->indizes '(0 1 2 3 4 5 6 7 8))))))

(define (check-quadranten spiel)
  (map check-duplicate (map (curry remove* (list 0 'X)) (map (curry spiel->einträge spiel) (map quadrant->indizes '(0 1 2 3 4 5 6 7 8))))))
