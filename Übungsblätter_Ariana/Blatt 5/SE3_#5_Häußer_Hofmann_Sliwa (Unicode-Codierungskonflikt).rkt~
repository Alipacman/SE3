...
#|
Blatt 5, Gruppe 8, Mittwoch 10-12 Uhr, Raum G-210, Übungsleiter Rainer Jürgensen
...
Wird dabei nicht #f aufgegeben, so wissen wir, dass merkmal2 in der sortierten Merkmalsliste weiter hinten steht als merkmal1, und somit ist merkmal2 dominanter
als merkmal1.
...
iii)
Den genetischen Grundlagen nach liegen für jedes Merkmal zwei Merkmalsausprägung auf unterschiedlichen Allelen vor. Dabei ist eine der Merkmalesausprägungen die
dominante, und eine die rezessive.
...
; --- Implementation ---
; Datenstruktur: Wir speichern alle möglichen Merkmalsausprägungen in insgesamt 4 Listen, wobei die Merkmale so geordnet sind, dass in aufsteigender Reihenfolge
; links die rezessiven und rechts die dominanten Merkmale stehen.
...
; Hilfsfunktion, die auf zwei Listen, die (neben der Merkmalsliste aller möglichen Mermale) jeweils die dominanteren Merkamle der zwei Listen in einer
; neuen Liste ausgibt
...
; Hilfsfunktion, die eines der beiden Allele (dominant oder rezessiv; werden als Parameter übergeben) random aussucht -> aus zwei Listen wird eine
;gemacht mit den Merkmalen, die an das Kind vererbt werden
...
; Die Funktion wendet auf die je zwei Listen der Eltern die randomize-Funktion an und auf die beiden Ergebnislisten wird der Dominanztest angewendet
; um den Phänotyp den Schmetterlingkindes heruaszufinden.
...
; Als Parameter werden die Merkmale aller Kinder  als Listen in einer Liste (kinderliste) übergegeben, sowie die dominaten Merkmale der Mutter und die
; des Vaters (je in einer Liste)
...
; Deshalb bekommt jedes Kind das Merkmal grün. Da grün die dominanteste der Flügelfarben ist, zeigt sich diese Merkmalsausprägung immer und unabhängig
; der von der Mutter geerbten Flügelfarbe.
...
;;;;;;;;; Lösung wie die Aufgabe es eigentlich vorsieht. (rezessive Merkmale werden zufällig ausgewählt)

; Hilfsfunktion
; Erstellt eine Liste rezessiver Merkmale zu einem Dominanten Merkmal.    
(define (member-reverse ele1 list1)
  (if (empty? list1)
      '()
      (if (equal? ele1 (car list1))
          (list (car list1))
          (cons (car list1) (member-reverse ele1 (cdr list1))))))

; Hilfsfunktion
; Erstellt eine Liste rezessiver Merkmale unter Beachtung der Dominanzregeln.
(define (pick-rndm-rez dominant merkmalsliste)
  (letrec ([ inner (lambda (rez dominant merkmalsliste)
                     (if (empty? dominant)
                         (reverse rez)
                         (inner (cons (car(shuffle(member-reverse (car dominant) (car merkmalsliste)))) rez)
                               (cdr dominant) (cdr merkmalsliste))))])
    (inner '() dominant merkmalsliste)))

; Hier müssen dann nur die dominanten Merkmale der Eltern übergeben werden.
(define (chewbaccafly1 vaterD vaterR mutterD mutterR)
      (dominanztestAufListen (randomize vaterD vaterR) (randomize mutterD mutterR) merkmalsliste))

; Hier müssen dann nur die dominanten Merkmale der Eltern übergeben werden.
(define (zeichneFamilie1 anzahlKinder vaterD mutterD)
  (let ([vaterR (pick-rndm-rez vaterD merkmalsliste)] [mutterR (pick-rndm-rez mutterD merkmalsliste)])
  (letrec ( [inner (lambda (i kinderliste)
                   (if (= i 0)
                       (zeichne kinderliste mutterD vaterD)
                       (inner (- i 1) (append kinderliste (list (chewbaccafly1 vaterD vaterR mutterD mutterR))))))])
                  (inner anzahlKinder '()))))

;Tests
;(zeichneFamilie1 8 '(green dots curved rhomb)  '(red star curly ellipse))
;(zeichneFamilie1 4 '(red dots curly ellipse) '(blue star curly ellipse))